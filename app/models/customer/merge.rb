class Customer < ActiveRecord::Base
  ...
  def self.save_and_update_foreign_keys!(c0, c1)
    ...
    begin
      transaction do
        old_user = true
        old_user = false if Authorization.find_by_provider_and_uid('Identity', c0.email)
        ...
        if c1.fresher_than?(c0)
          # c1 has identity
          if auth = Authorization.find_by_provider_and_uid('Identity', c1.email)
            email = c0.email
        ...
        if pass
          if old_user
            ...
            if salt
              ...
              # Customer's password is protected, cannot change
