module Customer::Search
  ...
  class_methods do
    ...
    def possible_matches(first,last,email=nil)
      [
    ...
    def match_last_name_and_substring_of_first_name(first,last,email)
    ...
    def email_matches_diff_last_name?(p)
    ...
    def email_last_name_match_diff_address?(p)
    ...
    def match_email_and_last_name(email,last_name)
    ...
    def match_first_last_and_address(first_name,last_name,street)
    ...
    def match_uniquely_on_names_only(first_name, last_name)
    ...
    def find_or_create!(cust, loggedin_id=0)
    ...
    def partial_name_matches(terms)
      conds =
        Array.new(terms.length, "(lower(first_name) LIKE ? or lower(last_name) LIKE ?)").join(' AND ')
      binds = terms.map { |w| ["%#{w.downcase}%", "%#{w.downcase}%"] }
      Customer.regular_customers.where(*([conds,binds].flatten)).distinct.order('last_name')
