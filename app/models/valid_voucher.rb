...
class ValidVoucher < ActiveRecord::Base
  ...
  def self.bundles(seasons = [Time.this_season, 1 + Time.this_season])
    ValidVoucher.where('vouchertypes.category = "bundle" AND vouchertypes.season IN (?)', seasons).
      include(:vouchertype).
  ...
  def self.bundles_available_to(customer = Customer.generic_customer, promo_code=nil)
    bundles = ValidVoucher.where('vouchertypes.category = "bundle" AND ? BETWEEN start_sales AND end_sales', Time.now).
      include(:vouchertype).
      order("season DESC,display_order,price DESC")
    bundles.map! do |b|
      b.customer = customer
