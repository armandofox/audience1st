class WalkupSalesController < ApplicationController
  ...
  def create
    ...
    (Rails.logger.warn "[WS]   finalize") rescue nil
    begin
      @order.finalize!
      (Rails.logger.warn "[WS] finalize done") rescue nil
      Txn.add_audit_record(:txn_type => 'tkt_purch',
        :customer_id => Customer.walkup_customer.id,
        :comments => 'walkup',
        :purchasemethod_id => p,
        :logged_in_id => current_user.id)
      (Rails.logger.warn "[WS] add_audit_record done") rescue nil
      ...
      (Rails.logger.warn "[WS] reporting error") rescue nil
