class CustomersController < ApplicationController
  ...
  before_filter(:is_logged_in,
                :except => %w[logout login new user_create])
  ...
  before_filter :is_staff_filter, :only => %w[list show]

  # only superadmin can destory customers, because that wreaks havoc
  before_filter :is_admin_filter, :only => ['destroy'], :redirect_to => {:action => :list}, :add_to_flash => 'Only super-admin can delete customer'
  ...
  def welcome
    # main screen for managing your account. 
    # This is what non-admins see on login,
    # but can also be reached by an admin 'acting on behalf of' a customer.
    @customer,@is_admin = for_customer(params[:id])
    @is_boxoffice = is_boxoffice
    @is_boxoffice_manager = is_boxoffice_manager
    redirect_to(:action => 'login') unless @customer.kind_of?(Customer)
  ...
  def login
    ...
    l.strip! rescue nil
    p = params[:customer][:password].to_s
    p.strip! rescue nil
    # if customer clicked 'forgot password' box, send email
    if  params[:forgot_password]
      if l.empty?
        flash[:notice] = "Please provide your login name (email address) "
        flash[:notice] << "to have your password emailed to you."
        logger.info("Empty forgot-password with pass=<#{p}>")
        return
      else
        return forgot_password(l)
      end
    end
    if (l.empty?) || (p.empty?)
      flash[:notice] = "Please provide both your login name (email address) "
      flash[:notice] << " and password."
      logger.info("Empty login or password: login=<#{l}> pass=<#{p}>")
      return
    end
    # try authenticate
    c = Customer.authenticate(l,p)
    if c.kind_of?(Customer)
      session[:cid] = c.id
      c.update_attribute(:last_login,Time.now)
      logger.info("Login successful <#{l}>")
      # if this is an admin-enabled login, inform user
      if c.is_boxoffice # the least privilege level that allows seeing other customer accts
      ...
      if @checkout_in_progress
        ...
        redirect_to :action => 'welcome', :id => c.id
      end
    else
      case c
      ...
      when :bad_password
        flash[:notice] = "We recognize your email address, but you mistyped " <<
          ...
          "click Continue, and we will email you your password."
  ...
  def logout
    ...
    redirect_to :action => 'login'       # we don't have a separate logout screen.
  end
  ...
  def forgot_password(login)
    if login.nil? || login.empty?
      flash[:notice] = "Please enter your email address to have your password mailed to you."
