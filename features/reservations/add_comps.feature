...
Scenario Outline: add comps to performance
  ...
  When I visit the add comps page for customer "admin"
  ...
  Then customer "admin" should have an order with comment "Courtesy Comp" containing the following tickets:
  | qty      | type | showdate       |
  | <number> | Comp | Apr 20, 8:00pm |
  And an email should be sent to customer "admin"
