...
describe VoucherPresenter do
  ...
  describe 'sorting', :focus => true do
    before(:each) do
    ...
    specify 'same showdates sort together' do
      v1 = create(:revenue_voucher, :vouchertype => @vt[0])
      v1.reserve!(@sd[0])      # earlier showdate
      v2 = create(:revenue_voucher, :vouchertype => @vt[0])
      v2.reserve!(@sd[1])       # same vtype as first, but later showdate
      v3 = create(:revenue_voucher, :vouchertype => @vt[1])
      v3.reserve!(@sd[0])      # same showdate
