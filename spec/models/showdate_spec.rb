  ...
  describe "availability grade" do
    ...
    cases.each do |c,grade|
      specify "with thresholds #{c.join ','}" do
        allow(Option).to_receive(:limited_availability_threshold).and_return(c[0])
        allow(Option).to_receive(:nearly_sold_out_threshold).and_return(c[1])
        allow(Option).to_receive(:sold_out_threshold).and_return(c[2])
    ...
    describe "revenue" do
      before(:each) do
        @showdate.vouchers.each do |v|
          allow(v).to_receive(:amount).and_return(11.00)
        end
      end
