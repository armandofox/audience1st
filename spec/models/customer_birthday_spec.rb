...
describe Customer, "birthdays" do
  ...
  describe "delivering email" do
    before :each do
      allow(Option).to_receive(:send_birthday_reminders).and_return(5)
      allow(Option).to_receive(:boxoffice_daemon_notify).and_return('n@ai')
    end
    context "should not be attempted" do
      ...
      it 'when feature is turned off' do
        allow(Option).to_receive(:send_birthday_reminders).and_return(0)
        Customer.notify_upcoming_birthdays
      end
      it 'when every-n value is negative' do
        allow(Option).to_receive(:send_birthday_reminders).and_return(-2)
        Customer.notify_upcoming_birthdays
      end
      it 'when no recipient specified in options' do
        allow(Option).to_receive(:boxoffice_daemon_notify).and_return('')
        Customer.notify_upcoming_birthdays
      end
      ...
      it 'when there are no customers with birthdays' do
        Timecop.travel(Date.parse('Jan 5, 2012')) do
          allow(Customer).to_receive(:birthdays_in_range).and_return([])
          Customer.notify_upcoming_birthdays
        end
      end
    end
    context "when there are customers with birthdays" do
      before :each do
        allow(Customer).to_receive(:birthdays_in_range).and_return([@c1,@c2])
      end
