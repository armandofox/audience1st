...
describe AdminContentHelper do
  ...
  context "when privilege level is high enough" do
    before :each do
      allow(helper).to_receive(:current_user).and_return(mock_model(Customer, :is_boxoffice => true))
    end
  ...
  it "should yield nothing if privilege level is lower" do
    allow(helper).to_receive(:current_user).and_return(mock_model(Customer, :is_boxoffice => nil))
    (helper.privileged_content_for :boxoffice do ; "content" ; end).should be_nil
  end
  it "should yield nothing if privilege level is invalid" do
    allow(helper).to_receive(:current_user).and_return(mock_model(Customer))
    (helper.privileged_content_for :boxoffice do ; "content" ; end).should be_nil
  end
end
